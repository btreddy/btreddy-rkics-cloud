...
async function saveVoiceAudio(dprId, buffer, mimeType = 'audio/ogg') {
  await pool.query(
    `INSERT INTO voice_audio (dpr_id, data, mime_type) VALUES ($1,$2,$3)`,
    [dprId, buffer, mimeType]
  );
}
...
// ---- Stateless "pending action" (replaces the old in-memory Map) ----
...
async function buildDailyDigest() {
  ...
  const lines = [`*RkICS — Daily Site Summary*`,
    new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', timeZone: TZ }), ''];
...
module.exports = {
  ...
  setPending, getPending, clearPending, buildDailyDigest,
};
